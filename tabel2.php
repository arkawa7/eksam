<?php require_once("functions.php");
	$array_of_names = getNikk2Data();
	
	if(isset($_POST["save"])){
	updateNikk1Data(($_POST["id"]), 1, $_POST["varnished"], 0);
	}
?>
...
<table border=1>
	...
	for($i = 0; $i < count($array_of_names); $i++){
		...
		echo "<td><input name='varnished' value='".$array_of_names[$i]->varnished."'></td>";
		...
		echo "<td>".$array_of_names[$i]->id."</td>";
		echo "<td>".$array_of_names[$i]->name."</td>";
		echo "<td>".$array_of_names[$i]->varnished."</td>";
...
<form action="tabel1.php" method="post" >
	<input type="submit" name="tabel1" value="Suunab treimise tabelisse">
</form>

<form action="tabel3.php" method="post" >
	<input type="submit" name="tabel3" value="Sunnab pakimise tabelisse">
</form>